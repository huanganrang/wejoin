...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/diveActivityCommentController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				field : 'activityId',
				title : '<%=TdiveActivityComment.ALIAS_ACTIVITY_ID%>',
				width : 50		
				}, {
				field : 'comment',
				title : '<%=TdiveActivityComment.ALIAS_COMMENT%>',
				width : 50		
				}, {
				field : 'pid',
				title : '<%=TdiveActivityComment.ALIAS_PID%>',
				width : 50		
				}, {
				field : 'userId',
				title : '<%=TdiveActivityComment.ALIAS_USER_ID%>',
				width : 50		
				}, {
				field : 'addtime',
				title : '<%=TdiveActivityComment.ALIAS_ADDTIME%>',
				width : 50		
			}, {
				field : 'action',
				title : '操作',
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/diveActivityCommentController/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/diveActivityCommentController/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/diveActivityCommentController/view?id=' + id
		});
	}
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/diveActivityCommentController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function downloadTable(){
	    ...
	    $('#downloadTable').form('submit', {
	        url:'${pageContext.request.contextPath}/diveActivityCommentController/download',
