...
DrawingBoard.Board = function(id, opts) {
	...
	var tpl = '</canvas><canvas class="drawing-board-canvas"></canvas><div class="drawing-board-cursor drawing-board-utils-hidden"></div>';
	/*
	...
	this.$el.find('.whiteboard:eq(0)').addClass('drawing-board-controls');
	this.$el.find('.whiteboard:eq(1)').addClass('drawing-board-canvas-wrapper').append(tpl);
	this.$el.addClass('drawing-board');
	this.dom = {
...
DrawingBoard.Board.prototype = {
	...
	resize: function() {
		...
		var widths = [
			this.dom.$canvasWrapper.width(),
			DrawingBoard.Utils.boxBorderWidth(this.dom.$canvasWrapper),
			DrawingBoard.Utils.boxBorderWidth(this.dom.$canvasWrapper, true, true)
		];
		var heights = [
			this.dom.$canvasWrapper.height(),
			DrawingBoard.Utils.boxBorderHeight(this.dom.$canvasWrapper),
			0,
			0,
		...
		if (this.opts.enlargeYourContainer) {
			...
			canvasWidth = this.dom.$canvasWrapper.width();
			canvasHeight = this.dom.$canvasWrapper.height();
		}
